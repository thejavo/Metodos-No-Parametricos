...
for ticker in tickers[:-1]:  # range hasta la penúltima columna
    ...
    resultados['var_e_hat'][ticker] = VAR_e_hat

    ESP_e_hat = np.mean(est_window['e_hat'])
    VAR_theta_hat = np.linalg.inv(X.T @ X) * VAR_e_hat

    resultados['covarianza_evento'][ticker] = VAR_theta_hat
    ...
    resultados['var_car'][ticker] = (gamma.T @ VAR_e_hat_star @ gamma)[0,0]
    ...
    SCAR_hat = resultados['car'][ticker] / (resultados['var_car'][ticker])**0.5
