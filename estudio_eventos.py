...
def estudio_eventos(fecha_evento, L1, indice_dia_evento, dias_previos, dias_posteriores, filename,
                    nombre_evento="Evento 1", gap_estimacion_evento=0, market_benchmark="^GSPC",
                    nivel_confianza=0.95, qdeunos=3, cerosantes=0, omitir_tickers=None):
    import pandas as pd
    import numpy as np
    ...
    activo_elegido = "YPFD.BA"  #para graficar MarketModel
    ...
    # --- LECTURA DE DATOS ---
    precios = pd.read_csv(filename, index_col="Date", parse_dates=["Date"])
    ...
    if omitir_tickers is None:
        omitir_tickers = []

    # Elimino los tickers a omitir del DataFrame de retornos
    columnas_validas = [col for col in retornos.columns if col not in omitir_tickers]
    retornos = retornos[columnas_validas]
    ...
    for i, ticker in enumerate(tickers):
        ...
        if est_window[market_benchmark].isna().sum() > 0:
            print(f"⚠️ Advertencia: {market_benchmark} tiene valores NaN en la ventana de estimación para la fecha {fecha_evento}. Omitiendo esta simulación.")
            return None
